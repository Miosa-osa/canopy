...
describe("isDirty logic (persona editor)", () => {
  ...
  it("is false after cancel (editMode becomes false)", () => {
    ...
    expect(computeIsDirty(editModeAfterCancel, "modified", "original")).toBe(
      false,
    );
  });
});
...
describe("⌘S / Ctrl+S handler contract", () => {
  ...
  it("triggers on ⌘S (macOS)", () => {
    expect(shouldTriggerSave({ metaKey: true, ctrlKey: false, key: "s" })).toBe(
      true,
    );
  });

  it("triggers on Ctrl+S (Windows/Linux)", () => {
    expect(shouldTriggerSave({ metaKey: false, ctrlKey: true, key: "s" })).toBe(
      true,
    );
  });
  ...
  it("does not trigger on plain S", () => {
    expect(
      shouldTriggerSave({ metaKey: false, ctrlKey: false, key: "s" }),
    ).toBe(false);
  });

  it("does not trigger on ⌘Enter", () => {
    expect(
      shouldTriggerSave({ metaKey: true, ctrlKey: false, key: "Enter" }),
    ).toBe(false);
  });
});
