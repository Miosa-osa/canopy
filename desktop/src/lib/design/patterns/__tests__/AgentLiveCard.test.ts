...
function resolveStatusLabel(
  ...
  switch (state) {
    ...
    case "thinking":
      return "Thinking\u2026";
    case "tool_call":
      return lastEntry?.kind === "tool_call"
        ? `Using: ${lastEntry.toolName}`
        : "Using tool\u2026";
    case "assistant":
      return "Responding\u2026";
    case "error":
      return lastEntry?.kind === "system"
        ? `Error: ${lastEntry.text}`
        : "Error";
    case "completed": {
...
describe("resolveDotColor", () => {
  it("maps thinking → green", () =>
    expect(resolveDotColor("thinking")).toBe("green"));
  it("maps tool_call → green", () =>
    expect(resolveDotColor("tool_call")).toBe("green"));
  it("maps assistant → green", () =>
    expect(resolveDotColor("assistant")).toBe("green"));
  it("maps completed → green", () =>
    expect(resolveDotColor("completed")).toBe("green"));
...
describe("resolveStatusLabel", () => {
  ...
  it("shows Thinking… for thinking state", () => {
    expect(resolveStatusLabel("thinking", makeThinking(), null, null)).toBe(
      "Thinking\u2026",
    );
  });

  it("shows Using: {toolName} for tool_call state", () => {
    expect(resolveStatusLabel("tool_call", makeToolCall(), null, null)).toBe(
      "Using: Bash",
    );
  });

  it("shows Responding… for assistant state", () => {
    expect(resolveStatusLabel("assistant", makeAssistant(), null, null)).toBe(
      "Responding\u2026",
    );
  });
  ...
  it("shows Error: {text} for system error", () => {
    ...
    expect(resolveStatusLabel("error", entry, null, null)).toBe(
