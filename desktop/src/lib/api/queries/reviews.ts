...
export function getReviewSummary(filters?: ReviewFilters): Promise<ReviewSummary> {
  const params = new URLSearchParams();
  if (filters?.workspaceSlug) params.set('workspace_slug', filters.workspaceSlug);
  if (filters?.status) params.set('status', filters.status);
  if (filters?.kind) params.set('kind', filters.kind);
...
export function approveReview(id: string, body?: ApproveReviewBody): Promise<Review> {
  return apiPost<Review>(`/reviews/${id}/approve`, body ?? {});
}

export function rejectReview(id: string, body?: RejectReviewBody): Promise<Review> {
  return apiPost<Review>(`/reviews/${id}/reject`, body ?? {});
}

export function requestChangesReview(id: string, body?: RequestChangesBody): Promise<Review> {
  return apiPost<Review>(`/reviews/${id}/request_changes`, body ?? {});
}

export function resubmitReview(id: string, attrs?: { artifactPreview?: string }): Promise<Review> {
...
export function approveReviewMutation() {
  return {
    ...
    mutationFn: ({ id, body }: { id: string; body?: ApproveReviewBody }) => approveReview(id, body),
  };
}
...
export function rejectReviewMutation() {
  return {
    ...
    mutationFn: ({ id, body }: { id: string; body?: RejectReviewBody }) => rejectReview(id, body),
  };
}
...
export function resubmitReviewMutation() {
  return {
    ...
    mutationFn: ({ id, attrs }: { id: string; attrs?: { artifactPreview?: string } }) =>
