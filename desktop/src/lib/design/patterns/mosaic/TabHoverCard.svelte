<script lang="ts">
  ...
  const command = $derived(
    asString(cfg.command) ?? asString(cfg.cmd) ?? pane.title,
  );
  const cwd = $derived(
    asString(cfg.cwd) ?? asString(cfg.working_directory) ?? null,
  );
  const branch = $derived(asString(cfg.branch));
  const agent = $derived(
    asString(cfg.agent) ?? asString(cfg.agent_slug) ?? null,
  );
  const runtime = $derived(
    asString(cfg.runtime) ?? asString(cfg.runtimeType) ?? null,
  );
  const model = $derived(asString(cfg.model));
  const startedAt = $derived(
    asString(cfg.started_at) ??
      asString(cfg.startedAt) ??
      asString(cfg.created_at) ??
      null,
  );
  const cost = $derived(
    asNumber(cfg.cost_usd) ??
      asNumber(cfg.cost) ??
      asNumber(cfg.total_cost) ??
      null,
  );
