...
const {
  ...
  const allTilesMock = vi.fn<() => Array<{ id: string; panes: Array<{ id: string }> }>>();
...
import { __test, buildDispatcher } from './build-dispatcher.svelte.js';
...
describe('deriveTitle', () => {
  it('prefers path', () => {
    expect(__test.deriveTitle('file_viewer', { path: 'lib/foo.ex' })).toBe('lib/foo.ex');
  });

  it('falls back to queued_command for terminal', () => {
    expect(__test.deriveTitle('terminal', { queued_command: 'mix test' })).toBe('mix test');
  });
