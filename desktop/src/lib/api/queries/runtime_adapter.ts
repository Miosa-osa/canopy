...
export function rolesQuery(opts: RolesQueryOpts = {}) {
  ...
  return {
    ...
    queryFn: () =>
      apiGet<{ data: ModelRoleAssignment[] }>(`/runtime-adapter/roles${qs}`).then((r) => r.data),
  };
}

export async function assignRole(body: RoleAssignmentCreate): Promise<ModelRoleAssignment> {
...
export function checkpointsQuery(sessionId: string, opts: CheckpointsQueryOpts = {}) {
...
export async function createCheckpoint(body: CheckpointCreate): Promise<RuntimeCheckpoint> {
...
export async function restoreCheckpoint(
  ...
  return apiPost<CheckpointRestoreResult>(`/runtime-adapter/checkpoints/${id}/restore`, {
...
export async function suggestRuntime(body: SuggestionRequest): Promise<SuggestionResult> {
