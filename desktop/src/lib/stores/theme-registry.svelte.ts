...
export function getCssVars(theme: ThemeDefinition): Record<string, string> {
  ...
  return {
    ...
    '--scrollbar-thumb-hover': isDark ? 'oklch(1 0 0 / 20%)' : 'oklch(0 0 0 / 25%)',
    ...
    '--glass-bg': isDark ? 'rgba(28, 28, 30, 0.8)' : 'rgba(255, 255, 255, 0.85)',
    '--glass-border': isDark ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.08)',
    '--glass-shadow': isDark
...
class ThemeRegistryStore {
  ...
  activeTheme = $derived(themes.find((t) => t.id === this.activeThemeId) ?? themes[0]);
  ...
  #applyToDOM(theme: ThemeDefinition): void {
    ...
    try {
      localStorage.setItem('canopy.theme.mode', theme.variant === 'dark' ? 'dark' : 'light');
    } catch {}
  }
}
