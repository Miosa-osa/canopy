...
async function serversUp(request: import('@playwright/test').APIRequestContext): Promise<boolean> {
  try {
...
const ROUTES: RouteCheck[] = [
  ...
  // REAL BUG: channels[0] accessed without null guard when data unavailable.
  {
    ...
    role: null,
    cssClass: 'cp-shell',
    knownBugs: ['BUG-001: channels[0] accessed without null guard — crashes when API unreachable'],
  },
  ...
  for (const route of ROUTES) {
    test(`[route] ${route.slug} → ${route.path}`, async ({ page, request }) => {
      ...
      if (route.role) {
        ...
        await expect(el, `CSS class *${route.cssClass}* on ${route.path}`).toBeVisible({
          timeout: 8000,
        });
      }
        ...
        ? real.filter(
            (e) =>
              ...
              !route.knownBugs!.some((bug) => {
                const isChannelsBug = bug.includes('BUG-001') && e.includes('0');
                const isIssuesBug = bug.includes('BUG-002') && e.includes('each_key');
                const isGoalsBug = bug.includes('BUG-003') && e.includes('charAt');
                const isTeamBug =
...
test.describe('Claude round-trip', () => {
  ...
  test('claude-roundtrip: spawn session → terminal alive → input → output', async ({
    ...
    await expect(newSessionBtn, 'Step 4: New session button visible').toBeVisible({
      timeout: 8000,
    });
    ...
    await page.waitForURL(/\/sessions\/[a-zA-Z0-9_-]+$/, { timeout: 20000 }).catch(() => null);

    const finalUrl = page.url();
    const navigatedToSession = /\/sessions\/[a-zA-Z0-9_-]+$/.test(finalUrl);
    expect(navigatedToSession, `Step 9: URL is /sessions/<id>, got: ${finalUrl}`).toBe(true);
    ...
    expect(hasBridgeBanner, 'Step 11: No "terminal bridge not ready" banner').toBe(0);
    ...
    const helloVisible = await page
      .waitForFunction(
        () => {
          ...
          return Array.from(rows).some((el) => el.textContent?.includes('hello'));
        },
        { timeout: 20000 }
      )
      .then(() => true)
      .catch(() => false);
    expect(helloVisible, 'Step 14: "hello" appears in terminal output').toBe(true);

    // Step 15: assert status shows "running"
    const statusRunning = page
      .getByText(/running/i)
      .or(page.locator('[class*="status"][class*="running"]'));
    await expect(statusRunning.first(), 'Step 15: Status "running"').toBeVisible({
      timeout: 5000,
    });
    ...
    if (await pauseBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      await expect(pausedState.first(), 'Step 16: Paused state visible').toBeVisible({
        timeout: 5000,
      });
      ...
      if (await resumeBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const runningAgain = page
          .getByText(/running/i)
          .or(page.locator('[class*="status"][class*="running"]'));
        await expect(runningAgain.first(), 'Step 17: Running again after resume').toBeVisible({
          timeout: 5000,
        });
      }
    }
    ...
    expect(real, `Real JS errors during round-trip: ${real.join('; ')}`).toHaveLength(0);
  });
});
...
test.describe('Kanban dispatch', () => {
  ...
  test('kanban-dispatch: drag card Todo → In progress', async ({ page, request }) => {
    const up = await serversUp(request);
    if (!up) {
    ...
    const inProgressCol = page
      ...
      .or(page.locator('[data-column="in_progress"], [data-column="in-progress"]'));
    ...
    if (!toastVisible) {
      console.warn('[kanban-dispatch] Toast not detected — drag may not be supported headless');
    }
    ...
    if ((await terminalIcon.count()) > 0) {
      console.log('[kanban-dispatch] Terminal icon found on card after dispatch');
    }
...
test.describe('Multi-board', () => {
  ...
  test('multi-board: create board → assert URL update', async ({ page, request }) => {
    const up = await serversUp(request);
    if (!up) {
    ...
    if (
      ...
      test.skip(true, 'BoardPicker not present — feature may not be implemented yet');
      return;
    }
    ...
    expect(page.url().includes('board='), 'URL updates to ?board=<id>').toBe(true);

    const boardName = page.getByText('Test board');
    const nameVisible = await boardName.isVisible({ timeout: 3000 }).catch(() => false);
