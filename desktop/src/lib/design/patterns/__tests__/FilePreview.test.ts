...
describe("resolveViewer() — size cap", () => {
  it("returns too-large for files > 50 MB regardless of MIME", () => {
    expect(resolveViewer("application/pdf", "report.pdf", LARGE)).toBe(
      "too-large",
    );
  });
  ...
  it("allows exactly 50 MB (boundary is exclusive)", () => {
    expect(resolveViewer("application/pdf", "ok.pdf", SIZE_CAP_BYTES)).toBe(
      "pdf",
    );
  });
});
...
describe("resolveViewer() — MIME routing", () => {
  ...
  it("routes application/vnd.ms-excel to xlsx", () => {
    expect(resolveViewer("application/vnd.ms-excel", "sheet.xls", SMALL)).toBe(
      "xlsx",
    );
  });
  ...
  it("returns unsupported for unknown MIME and no matching extension", () => {
    expect(resolveViewer("application/octet-stream", "binary.bin", SMALL)).toBe(
      "unsupported",
    );
  });

  it("returns unsupported for zip files", () => {
    expect(resolveViewer("application/zip", "archive.zip", SMALL)).toBe(
      "unsupported",
    );
  });
});
...
describe("resolveViewer() — extension fallback", () => {
  ...
  it("returns unsupported for unrecognized extension", () => {
    expect(resolveViewer(GENERIC, "file.unknown123", SMALL)).toBe(
      "unsupported",
    );
  });
