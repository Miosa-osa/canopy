...
3. **Capability/Resource Removal** - list what the offer REMOVES from the buyer's required effort
...
2. **Derivative desire** - "The outcome is connected to my deeper desire"
3. **Can-fulfill belief** - "This offer CAN deliver"
4. **Urgency belief** - "I need this now, not later"
5. **This-offer belief** - "This specific offer is the right one" (vs generic market)
6. **Better/faster belief** - "This is better/faster than my alternatives"
...
1. **Price the identity, not the deliverables.** Cost-plus pricing = race to bottom. Identity-aligned = who buys at this price, and what do they require to feel congruent? $10K buyer requires exclusive framing; $50K buyer requires bespoke.
...
5. **Value stack itemization anchors pricing.** Show the price of each component separately - even if bundled. "If you hired this separately: $X + $Y + $Z = $17,400. You pay $4,997." Specificity + anchoring.

6. **Don't bundle the missing capability into the bonus - bundle it into the CORE.** If the ICP has a capability gap (from Section 8 of ICP doc), the CORE offer must solve it. Bonuses are for objection-countering, not capability-fixing.

7. **The guarantee creates the sale, then the offer delivers.** A weak guarantee means you're asking them to take the risk. A strong guarantee means YOU take the risk. The stronger the guarantee, the higher you can price.

8. **Continuity = LTV multiplier.** A $5K one-time core offer with $297/mo continuity (30% opt-in) has LTV 2.5 the standalone. Design continuity INTO the core, not as an afterthought.

9. **Upsells are identity ladders.** Core offer buyer -> identity shift -> qualified for next tier. Upsell sequence maps to identity progression, not just higher price.

10. **Test the offer against the 3am test (reverse).** "Would the ICP wake up at 3am thinking about this offer and be excited to buy tomorrow?" If no, the offer doesn't resonate with Layer 3 private desire. Revise.

## Process (numbered, sequential)

### Phase 0 - Load Dependencies
- Read `output/build-icp/` (latest ICP Document; require Completeness >= 80)
- Read `output/build-positioning/` if exists (else proceed with ICP alone)
...
- Recommended next skill (usually `/build-vsl`)
...
- **NEVER set pricing without creator sign-off** per `company.yaml` governance (pricing_change requires board approval).
...
- [ ] Value Equation Score >= 150
- [ ] LTV:CAC >= 3.0 (or fix paths documented)
- [ ] Mechanism named
- [ ] 5+ bonuses each mapped to ICP objection
- [ ] Guarantee is ROI-positive
- [ ] Limiting belief aligned to transformation type
- [ ] All 8 required beliefs have installation points
- [ ] Economics math shown (not estimated)
- [ ] Competitor whitespace identified
- [ ] Banned vocabulary check passed
- [ ] Triple-layer S/N score >= 0.8
- [ ] Creator sign-off on pricing + guarantee
