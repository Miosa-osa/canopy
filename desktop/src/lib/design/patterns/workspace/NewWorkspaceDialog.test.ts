...
// Test the same slug helper used by the dialog.
...
describe('slugify()', () => {
  ...
  it('matches the server-side slug regex', () => {
    ...
    const samples = ['my-workspace', 'sprint-2026-04', 'a-b', 'robert-s-project'];
...
describe('form validation contract', () => {
  ...
  function validate(workspaceName: string, selectedPath: string | null): string | null {
  ...
  it('rejects a missing folder', () => {
    expect(validate('OK', null)).toBe('Choose a folder to bind this workspace to.');
    expect(validate('OK', '')).toBe('Choose a folder to bind this workspace to.');
  });
