...
import { discardHunkMutation, stageFileMutation } from '$lib/api/queries/diff.js';
...
function handleDiscard(hunk: DiffHunk): void {
  ...
  $discardMut.mutate(
    ...
    },
    {
      ...
      onError: (err) => {
        toasts.error(err instanceof Error ? err.message : 'Failed to discard hunk');
      },
      onSettled: () => {
...
function handleStage(hunk: DiffHunk): void {
  ...
  $stageMut.mutate(
    { files: [file.path] },
    {
      ...
      onError: (err) => {
        toasts.error(err instanceof Error ? err.message : 'Failed to stage file');
      },
      onSettled: () => {
        stagingHeader = null;
      },
