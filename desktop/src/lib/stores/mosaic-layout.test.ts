...
const { getWorkspaceStateMock, putWorkspaceStateMock } = vi.hoisted(() => {
  const getWorkspaceStateMock = vi.fn<(slug: string, key: string) => Promise<unknown>>();
  const putWorkspaceStateMock =
    vi.fn<(slug: string, key: string, value: unknown) => Promise<unknown>>();

  return { getWorkspaceStateMock, putWorkspaceStateMock };
});
...
describe('per-workspace layout swap (localStorage)', () => {
  ...
  it('falls back to default layout on malformed JSON in storage', () => {
    ...
    try {
      loaded = JSON.parse(localStorage.getItem(lsKey('corrupt')) ?? '') as MosaicLayout;
    } catch {
