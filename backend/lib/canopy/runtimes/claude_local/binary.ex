defmodule Canopy.Runtimes.ClaudeLocal.Binary do
  @moduledoc """
  Binary resolution and path-safety layer for the Claude Local adapter.

  Handles three concerns:

  1. **Resolution** — resolves the `claude` binary from the execution context
  ...
  3. **Version check** — runs `claude --version` to confirm the binary is
  ...
  @doc """
  Resolves the `claude` binary path from an execution context map.
  ...
  on `$PATH`, the bare string `"claude"` is returned so the caller receives a
