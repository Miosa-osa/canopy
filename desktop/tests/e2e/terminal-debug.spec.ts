...
test('terminal bridge: xterm renders, fallback banner absent', async ({ page }) => {
  // ── 1. Create interactive session ──────────────────────────────────────────
  const resp = await page.request.post('http://localhost:9190/api/v1/sessions', {
    data: {
  ...
  const fallbackVisible = await page.locator('text=Terminal bridge not ready').isVisible();
  ...
  const wsJoinOk = wsFrames.some((f) => f.includes('phx_reply') && f.includes('"ok"'));
