...
describe('addMemberMutation()', () => {
  it('returns mutationKey ["channels", "members", "add"]', () => {
    expect(addMemberMutation().mutationKey).toEqual(['channels', 'members', 'add']);
  });
...
describe('removeMemberMutation()', () => {
  it('returns mutationKey ["channels", "members", "remove"]', () => {
    expect(removeMemberMutation().mutationKey).toEqual(['channels', 'members', 'remove']);
  });
...
describe('channelMessagesQuery()', () => {
  ...
  it('includes opts in query key', () => {
    ...
    expect(q.queryKey).toEqual(['channels', 'chan-1', 'messages', { limit: 50 }]);
  });
...
describe('markReadMutation()', () => {
  it('returns channel-scoped mutationKey', () => {
    expect(markReadMutation('chan-1').mutationKey).toEqual(['channels', 'chan-1', 'read']);
  });
