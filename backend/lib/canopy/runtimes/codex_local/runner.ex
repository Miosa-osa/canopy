defmodule Canopy.Runtimes.CodexLocal.Runner do
  ...
  1. Receives opts via `start_link/1`; spawns the Port on `init/1` so the
     supervisor gets a clean `{:ok, pid}` only after the process is ready.
  ...
  When Codex exits non-zero and the stdout+stderr contain a stale-session
  error pattern (detected by `Parser.unknown_session_error?/2`), the Runner
  emits a `:system` entry with `event: "session_expired"`. The caller should
  clear the stored session ID and re-execute with a fresh context. Automatic
  retry is deliberately NOT performed inside the Runner — the caller (or
  LiveView handler) owns the retry decision so as not to mask failures.

  ## Line buffering

  Codex's `--json` flag emits one JSON object per line. The Port may split
  lines across data packets; the Runner buffers incomplete lines and only
  parses when a newline is received.

  ## Restart strategy

  `:transient` — crashed Runners are NOT restarted. A mid-execution crash
  likely indicates the Codex process itself failed. The session is marked
  failed via PubSub + DB, and the user can start a new session.

  # TODO: shared runner
  # Week 2 refactor: consider extracting a generic `Canopy.Runtimes.ProcessRunner`
  # behaviour shared by Claude, Codex, and Gemini adapters. The core loop is
  # identical; only the Parser module and env/args differ. Defer until all three
  # adapters are stable and the shared surface area is clear.
  """
  ...
  def init(opts) do
    ...
    Logger.info("[CodexLocal.Runner] starting session=#{session_id} binary=#{binary}")
  ...
  def handle_info({port, {:exit_status, code}}, %{port: port} = state) do
    new_state = flush_buffer(state)
    Logger.warning("[CodexLocal.Runner] session=#{state.session_id} exited with code #{code}")

    # Surface stale-session errors so callers can clear the stored session ID
    # and retry with a fresh context instead of surfacing a generic failure.
    if Parser.unknown_session_error?(state.line_buffer, state.stderr_buffer) do
      emit_system_entry(new_state, "session_expired", %{exit_code: code})
    else
  ...
  defp emit_entry(state, entry) do
    ...
    case Canopy.Sessions.append_message(state.session_id, stamped) do
      ...
      {:error, reason} ->
        Logger.warning(
          "[CodexLocal.Runner] append_message failed session=#{state.session_id}: #{inspect(reason)}"
        )
    end
