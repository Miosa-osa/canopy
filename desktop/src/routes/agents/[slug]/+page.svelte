...
import { type Budget, budgetsQuery } from '$lib/api/queries/budgets.js';
...
function loadAgentDraft(currentAgent: AgentDetail) {
  ...
  draft = {
    ...
    contextTier:
      (saved.contextTier ??
        currentAgent.contextTier ??
        configString(currentAgent.config, 'context_tier')) ||
      'l1',
    skillsText:
      saved.skillsText ??
      (currentAgent.skills ?? configList(currentAgent.config, 'skills')).join('\n'),
    toolsText:
      saved.toolsText ??
      (currentAgent.tools ?? configList(currentAgent.config, 'tools')).join('\n'),
  };
}
...
const isWorkspaceAgent = $derived(
  agent?.config?.source === 'workspace' || agent?.personaPath?.startsWith('workspace:')
);
...
function openAgentKanban() {
  let board = kanbanBoards.boards.find((b) => b.scope.type === 'agent' && b.scope.agentId === slug);
  if (!board) {
    board = kanbanBoards.createBoard(`${agent?.name ?? slug} tasks`, {
      type: 'agent',
      agentId: slug,
    });
  } else {
