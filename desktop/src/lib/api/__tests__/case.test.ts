...
describe('toCamel()', () => {
  it('converts a flat snake_case object to camelCase', () => {
    expect(toCamel({ agent_slug: 'dev', runtime_type: 'claude-local' })).toEqual({
      agentSlug: 'dev',
  ...
  it('converts nested objects recursively', () => {
    expect(toCamel({ session_detail: { started_at: '2026-01-01', cost_usd: 0.01 } })).toEqual({
      sessionDetail: { startedAt: '2026-01-01', costUsd: 0.01 },
    });
  });
...
describe('toSnake()', () => {
  ...
  it('converts nested objects recursively', () => {
    expect(toSnake({ sessionDetail: { startedAt: '2026-01-01', costUsd: 0.01 } })).toEqual({
      session_detail: { started_at: '2026-01-01', cost_usd: 0.01 },
    });
  });
