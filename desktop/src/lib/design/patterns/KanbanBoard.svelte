...
function handleFinalize(status: TaskStatus, e: CustomEvent<DndEvent<Task>>) {
  ...
  const moved = newItems.find((t) => t.id !== SHADOW_PLACEHOLDER_ITEM_ID && t.status !== status);
  ...
  // Optimistic update
  const updatedItems = newItems.map((t) => (t.id === moved.id ? { ...t, status } : t));
  ...
  void (async () => {
    try {
      ...
      const isNotFound =
        err instanceof Error && (err.message.includes('404') || err.message.includes('HTTP 404'));
      if (isNotFound) {
