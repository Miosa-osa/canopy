...
export function dispatchToolCall(
  ...
  return apiPost<ToolDispatchResponse>(
    `/agents/tools/${encodeURIComponent(toolName)}`,
    body,
  );
}
...
export function listToolInvocations(opts?: {
  ...
  const qs = buildQuery(opts ?? {});
  return apiGet<{ data: ToolInvocation[] }>(`/mcp/tool-calls${qs}`).then(
    (r) => r.data,
  );
}
...
export function registeredToolsQuery(opts?: {
  mcpExposed?: boolean;
  promptExposed?: boolean;
}) {
  return {
