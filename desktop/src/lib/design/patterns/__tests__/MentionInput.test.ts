...
describe('buildResults() — category aggregation', () => {
  it('returns results across all 4 categories for a broad query', () => {
    const results = buildResults('de', mockAgents, mockWorkspaces, mockTasks, mockChannels);
  ...
  it('caps workspaces at 2 results', () => {
    const results = buildResults('', mockAgents, mockWorkspaces, mockTasks, mockChannels);
  ...
  it('caps channels at 2 results', () => {
    const results = buildResults('', mockAgents, mockWorkspaces, mockTasks, mockChannels);
    const channelCount = results.filter((r) => r.category === 'channels').length;
  ...
  it('returns empty when no query matches anything', () => {
    const results = buildResults('zzz', mockAgents, mockWorkspaces, mockTasks, mockChannels);
    expect(results.length).toBe(0);
  });

  it('agent slug is used as mention slug', () => {
    const results = buildResults('code', mockAgents, mockWorkspaces, mockTasks, mockChannels);
  ...
  it('task slug is lowercased shortId', () => {
    const results = buildResults('T-1', mockAgents, mockWorkspaces, mockTasks, mockChannels);
...
describe('extractMentions() — backend regex compatibility', () => {
  ...
  it('outputs match backend regex for single mention', () => {
    ...
    const backendMatches = Array.from(text.matchAll(BACKEND_REGEX), (m) => m[1]);
