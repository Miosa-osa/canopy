...
describe('detectType() — markdown', () => {
  it.each([['README.md'], ['NOTES.markdown'], ['doc.mdx']])('%s → markdown', (name) => {
...
describe('detectType() — media', () => {
  it.each([['song.mp3'], ['voice.wav'], ['pod.flac'], ['talk.opus']])('%s → audio', (name) => {
    expect(detectType(name, null).viewer).toBe('audio');
  });

  it.each([['clip.mp4'], ['clip.webm'], ['clip.mov'], ['clip.mkv']])('%s → video', (name) => {
...
describe('detectType() — office', () => {
  ...
  it('DOCX MIME → docx', () => {
    expect(
      detectType('noext', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')
        .viewer
    ).toBe('docx');
  });
  ...
  it('XLSX MIME → xlsx', () => {
    expect(
      detectType('noext', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        .viewer
    ).toBe('xlsx');
  });
});
...
describe('detectType() — hex fallback', () => {
  ...
  it('binary MIME with unknown ext → hex', () => {
    expect(detectType('blob.bin', 'application/octet-stream').viewer).toBe('hex');
  });
});
...
describe('detectType() — size cap', () => {
  it('files over 50 MB → too-large regardless of type', () => {
    ...
    expect(detectType('big.png', 'image/png', oneHundredMB).viewer).toBe('too-large');
  });
