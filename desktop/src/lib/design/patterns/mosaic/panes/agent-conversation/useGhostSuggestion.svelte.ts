...
function resolve(draft: string): GhostSuggestion | null {
  ...
  if (trimmed.startsWith('/')) {
    ...
    const match = FALLBACK_BUILTINS.find((cmd) => cmd.name.startsWith(lower) && cmd.name !== lower);
    if (match) {
      return {
        text: match.name,
  ...
  const recentMatch = recent.find(
    (cmd) => cmd.toLowerCase().startsWith(trimmed.toLowerCase()) && cmd.length > trimmed.length
  );
  if (recentMatch) {
    return {
      text: recentMatch,
