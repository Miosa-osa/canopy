defmodule Canopy.Runtimes.GeminiLocal.Binary do
  @moduledoc """
  Binary resolution and path-safety layer for the Gemini Local adapter.

  Three concerns:

  1. **Resolution** — resolves the `gemini` binary from the execution context
     or from `$PATH` when no explicit path is configured.

  2. **Path-traversal guard** — rejects binary paths containing `..` components,
     preventing path-traversal attacks in user-supplied config.

  3. **Version check** — runs `gemini --version` to confirm the binary is
  ...
  @doc """
  Resolves the `gemini` binary path from an execution context map.
  ...
  on `$PATH`, the bare string `"gemini"` is returned so the caller receives a
