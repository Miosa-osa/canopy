<script lang="ts">
  ...
  import { sessionDetailQuery } from '$lib/api/queries/sessions.js';
  ...
  const createConversation = createMutation<Session, Error, { prompt: string }>({
    ...
    onSuccess: (s) => {
      const id = (s as unknown as { sessionId?: string; id?: string }).sessionId
  ...
  async function ensureConversationSession(prompt: string, selectedRuntime: string | null): Promise<string> {
    ...
    const id = (session as unknown as { sessionId?: string; id?: string }).sessionId
  ...
  function handleSlashCommand(command: BuildCommand): boolean {
    ...
    if (command.source === 'drive_prompt' || command.source === 'template' || command.source === 'skill') {
      draft = `${name} `;
      return false;
    }
  ...
  async function handleSlashSubmit(prompt: string): Promise<boolean> {
    ...
    if (trimmed.startsWith('/plan')) {
      void startRuntimeForShell(`Create an implementation plan. ${commandTail('/plan', trimmed)}`, null);
      return true;
    }
    if (trimmed === '/agent') {
      sessionId = null;
