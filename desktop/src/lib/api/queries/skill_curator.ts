...
export function lockfileQuery(opts: LockfileFilters = {}) {
  ...
  return {
    ...
    queryFn: () =>
      apiGet<{ data: LockfileEntry[] }>(`/skill-curator/lockfile${qs}`).then((r) => r.data),
  };
}
...
export async function unlockSkill(
  ...
  return apiDelete<LockfileEntry>(`/skill-curator/lockfile/${workspaceSlug}/${skillSlug}`);
}
...
export async function diffVersions(slug: string, from: string, to: string): Promise<VersionDiff> {
  const params = new URLSearchParams({ from, to });
  return apiGet<VersionDiff>(`/skill-curator/skills/${slug}/diff?${params.toString()}`);
}

// ── Verification ─────────────────────────────────────────────────────────────

export async function verifySkill(slug: string, verifiedBy: string): Promise<Verification> {
...
export function sourcesQuery() {
  return {
    ...
    queryFn: () =>
      apiGet<{ sources: RegistrySource[] }>('/skill-curator/sources').then((r) => r.sources),
  };
}

export async function addSource(body: RegistrySourceCreate): Promise<RegistrySource> {
...
export async function refreshSources(source?: string): Promise<RefreshResult> {
  return apiPost<RefreshResult>('/skill-curator/sources/refresh', source ? { source } : {});
}
