...
describe('paneTitle()', () => {
  ...
  it("returns the command for the 'command' format", () => {
    const out = paneTitle({ ...basePane, config: { command: 'claude code' } }, 'command');
    expect(out).toBe('claude code');
  });

  it('falls back to pane.title when command is missing', () => {
    expect(paneTitle({ ...basePane, config: {} }, 'command')).toBe('fallback title');
  });
  ...
  it('returns the branch when present', () => {
    const out = paneTitle({ ...basePane, config: { branch: 'feature/mosaic-polish' } }, 'branch');
    expect(out).toBe('feature/mosaic-polish');
  });

  it('falls back to command when branch is missing', () => {
    const out = paneTitle({ ...basePane, config: { command: 'claude code' } }, 'branch');
    expect(out).toBe('claude code');
  });

  it('falls back to pane.title when both branch and command are missing', () => {
    expect(paneTitle({ ...basePane, config: {} }, 'branch')).toBe('fallback title');
  });
