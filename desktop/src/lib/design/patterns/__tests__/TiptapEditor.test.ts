...
const mockWorkspaces: Workspace[] = [
  { slug: "dev-shop", name: "Dev Shop" } as Workspace,
];
...
describe("filterSlashCommands()", () => {
  ...
  it("filters by label match", () => {
    ...
    expect(
      results.every((c) => c.label.toLowerCase().includes("heading")),
    ).toBe(true);
  });
...
describe("extractMentions()", () => {
  it("extracts @mention at start of string", () => {
    expect(extractMentions("@code-reviewer check this")).toContain(
      "code-reviewer",
    );
  });
...
describe("slash command filter — debounce stub", () => {
  ...
  it("only fires callback once after 150ms debounce", () => {
    ...
    expect(result.every((c) => c.label.toLowerCase().includes("hea"))).toBe(
      true,
    );
  });
});
